...
def curriculum_structure_aware_PL(P, top_percent, device=None, reg_e=0.01):
    device_id = set_device(device)
    a = torch.ones((P.shape[0],), dtype=torch.float64).to(device_id) / P.shape[0]
    top_percent = min(torch.sum(a).item(), top_percent)
    b = torch.ones((P.shape[1],), dtype=torch.float64).to(device_id) / P.shape[1] * top_percent
    P = P.double()
...
def robust_train(net, model_ema, ema, optimizer, trainloader, n_classes, candidate_count, config, train_loss_meter,train_accuracy_meter):
    net.eval()
    pbar = tqdm(trainloader, ncols=150, ascii=' >', leave=False, desc='Optimal Transport')
    device = set_device(config.gpu)

    all_argmax_plabels = torch.zeros((len(trainloader.dataset),), dtype=torch.int64).cuda()
    with torch.no_grad():
        data_size = len(trainloader.dataset)
        all_out = torch.zeros((data_size, n_classes), dtype=torch.float32)

        for it, sample in enumerate(pbar):
            indices = sample['index']
            x = sample['data'][0].to(device)  # 只取需要的增强版本

            outputs = net(x)
            logits = outputs['logits'] if isinstance(outputs, dict) else outputs
            out = torch.softmax(logits, dim=-1)

            all_out[indices] = out.cpu()

            del outputs, logits, out

        all_out = all_out.to(device)
        P = F.log_softmax(all_out, dim=1)

        budget, pho = 1.0, 1.0

        couplings = curriculum_structure_aware_PL(P, top_percent=budget, reg_e=0.01, device=config.gpu)

        confidence, argmax_plabels = torch.max(couplings, dim=1)

        all_argmax_plabels.scatter_(0, torch.arange(data_size).to(device), argmax_plabels)

        del all_out, P, couplings
    net.train()
    pbar = tqdm(trainloader, ncols=150, ascii=' >', leave=False, desc='ROBUST TRAINING')

    for it, sample in enumerate(pbar):
        if config.dataset.startswith('cif'):
            indices = sample['index'].to(device)
            x, x_s = sample['data']
            x, x_s = x.to(device), x_s.to(device)
            y = sample['label'].to(device)

            candidate_label = torch.full(size=(y.size(0), n_classes), fill_value=0).cuda()
            candidate_label.scatter_(dim=1, index=torch.unsqueeze(y, dim=1), value=1)
            ...
            candidate_label.scatter_(dim=1, index=torch.unsqueeze(pesudo, dim=1), value=1)
            ...
            if config.label_smoothing == 1.0:
                ...
                L_PLL = F.cross_entropy(logits, y_ce.cuda(),label_smoothing=config.label_smoothing)
            ...
            sel = confidence[indices] > torch.mean(confidence[indices])
            if config.label_smoothing == 1.0:
                L_SSL = torch.mean(F.cross_entropy(logits[sel], all_argmax_plabels[indices][sel],reduction="none") * confidence[indices][sel]) *0.4  + F.cross_entropy(logits[sel],y[sel]) * 0.6 # + F.cross_entropy(logits[sel],y_ce.cuda()[sel]) * 0.1
            else:
                L_SSL = torch.mean(F.cross_entropy(logits[sel], all_argmax_plabels[indices][sel],reduction="none",label_smoothing=config.label_smoothing) * confidence[indices][sel]) *0.9 + F.cross_entropy(logits[sel],y[sel],label_smoothing=config.label_smoothing) * 0.1
            ...
            if config.label_smoothing == 1.0:
                ...
                L_CR = F.cross_entropy(logits_s, pesudo, label_smoothing=config.label_smoothing)

            loss = config.alpha * L_NL + config.beta * L_CR + config.gamma * L_PLL + config.delta * L_SSL
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        else:
            with autocast():
                ...
                sel = confidence[indices] > torch.mean(confidence[indices])
                if config.label_smoothing == 1.0:
                    L_SSL = torch.mean(F.cross_entropy(logits[sel], all_argmax_plabels[indices][sel],reduction="none") * confidence[indices][sel]) *0.1  + F.cross_entropy(logits[sel],y[sel]) * 0.9 # + F.cross_entropy(logits[sel],y_ce.cuda()[sel]) * 0.1
                else:
                    L_SSL = torch.mean(F.cross_entropy(logits[sel], all_argmax_plabels[indices][sel],reduction="none",label_smoothing=config.label_smoothing) * confidence[indices][sel]) *0.9 + F.cross_entropy(logits[sel],y[sel],label_smoothing=config.label_smoothing) * 0.1
...
def warmup(epoch, net, model_ema, ema, optimizer, trainloader, num_classes, candidate_count, train_loss_meter, train_accuracy_meter, config):
    net.eval()
    pbar = tqdm(trainloader, ncols=150, ascii=' >', leave=False, desc='Optimal Transport')
    device = set_device(config.gpu)
    ...
    with torch.no_grad():
        data_size = len(trainloader.dataset)
        all_out = torch.zeros((data_size, num_classes), dtype=torch.float32)

        for it, sample in enumerate(pbar):
            indices = sample['index']
            x = sample['data'][0].to(device)
        ...
        max_value, argmax_plabels = torch.max(couplings, dim=1)
...
def build_loader(params):
    ...
    if dataset_name.startswith('cif'):
        if params.noise_type == 'badlabel':
            dataset = build_cifar100n_badlabel_dataset("./data/cifar100",
        ...
        elif params.noise_type == 'IDN':
            dataset = build_cifar100n_badlabel_dataset("./data/cifar100",
                                                       CLDataTransform(transform['cifar_train'],
                                                                       transform['cifar_train_strong_aug']),
                                                       transform['cifar_test'], noise_type=params.noise_type,
                                                       openset_ratio=params.openset_ratio,
                                                       closeset_ratio=params.closeset_ratio)
    ...
    if dataset_name.startswith('food'):
        ...
        dataset = build_food101n_dataset('Datasets/food-101', CLDataTransform(transform['train_clothing1m'],
                                                                              transform["train_strong_aug_clothing1m"]),
                                         transform['test_clothing1m'])
        trainloader = DataLoader(dataset["train"], batch_size=params.batch_size, shuffle=True, num_workers=16,
                                 pin_memory=True)
...
def parse_args():
    ...
    parser.add_argument('--aph', type=float, default=0.99)
...
if __name__ == '__main__':
    ...
    assert config.dataset in ['cifar100nc', 'cifar80no', 'web-aircraft', 'web-bird', 'web-car', 'food101n']
    assert config.noise_type in ['symmetric', 'asymmetric', "IDN", 'badlabel']
    ...
    model_ema = copy.deepcopy(compiled_model)
    ...
    while epoch < config.epochs:
        ...
        if epoch < config.warmup_epochs:
            warmup(epoch, compiled_model, model_ema, ema, optim, input_loader,n_classes, candidate_count, train_loss_meter, train_accuracy_meter, config)
        else:
            robust_train(compiled_model, model_ema, ema, optim, input_loader, n_classes, candidate_count, config, train_loss_meter, train_accuracy_meter)
