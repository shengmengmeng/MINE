...
def robust_train(net, model_ema, ema, optimizer, trainloader, n_classes, candidate_count, config, train_loss_meter,train_accuracy_meter):
    ...
    confidence = torch.zeros((len(trainloader.dataset),), dtype=torch.float64).cuda()
    ...
    couplings = curriculum_structure_aware_PL(P.detach(), top_percent=budget, reg_e=0.01, device=config.gpu)
    ...
    for it, sample in enumerate(pbar):
        if dataset_name.startswith('cif'):
            ...
            y_ce = candidate_count[indices] / torch.sum(candidate_count[indices], dim=-1)[:, None]
            ...
            if config.label_smoothing == 1.0:
                L_SSL = torch.mean(F.cross_entropy(logits[sel], all_argmax_plabels[indices][sel],reduction="none") * confidence[indices][sel]) *0.1  + F.cross_entropy(logits[sel],y[sel]) * 0.9
            else:
            ...
            with autocast():
                ...
                sel = confidence[indices] > torch.mean(confidence[indices])
                if config.label_smoothing == 1.0:
                    L_SSL = torch.mean(F.cross_entropy(logits[sel], all_argmax_plabels[indices][sel],reduction="none") * confidence[indices][sel]) *0.9  + F.cross_entropy(logits[sel],y[sel]) * 0.1
                else:
                    L_SSL = torch.mean(F.cross_entropy(logits[sel], all_argmax_plabels[indices][sel],reduction="none",label_smoothing=config.label_smoothing) * confidence[indices][sel]) *0.9 + F.cross_entropy(logits[sel],y[sel],label_smoothing=config.label_smoothing) * 0.1
...
def warmup(epoch, net, model_ema, ema, optimizer, trainloader, candidate_count, train_loss_meter, train_accuracy_meter, config):
    net.eval()
    pbar = tqdm(trainloader, ncols=150, ascii=' >', leave=False, desc='Optimal Transport')
    ...
    with torch.no_grad():
        ...
        budget, pho = curriculum_scheduler(epoch, config.warmup_epochs, begin=0.3, end=1,
                                               mode='linear')
...
if __name__ == '__main__':
    ...
    assert config.dataset in ['cifar100nc', 'cifar80no', 'web-aircraft', 'web-bird', 'web-car']
    ...
    while epoch < config.epochs:
        ...
        if epoch < config.warmup_epochs:
            warmup(epoch, compiled_model, model_ema, ema, optim, input_loader, candidate_count, train_loss_meter, train_accuracy_meter, config)
        else:
