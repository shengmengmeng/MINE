...
**Abstract:** Deep neural networks are highly vulnerable to mislabeled data, a pervasive issue in large-scale datasets collected via automated web crawling. Existing label-noise learning (LNL) methods typically attempt to extract reliable supervision from corrupted annotations. However, this direct reliance induces dual-level confirmation biases: at the local instance level, ambiguous samples are overfit, and at the global class level, easier categories dominate learning, collectively distorting the optimization trajectory. To this end, we propose Multi-paradigm INdirect supErvision (MINE), a noise-tolerant framework that alleviates the reliance on unreliable labels by decomposing learning into complementary indirect supervision signals. Concretely, we reformulate LNL through a principled label-space decomposition, integrating partial-label learning and negative learning to constrain the prediction space without enforcing exact label matching. Building on this foundation, we introduce a global alignment mechanism via optimal transport to enforce structurally consistent label assignments, and a local consistency regularization scheme to anchor representation learning on intrinsic visual invariance. By unifying these indirect supervision paradigms, MINE effectively mitigates both local- and global-level confirmation biases, breaking the error-reinforcing cycles that plague conventional methods. Extensive experiments on synthetic and real-world noisy datasets demonstrate that MINE consistently outperforms state-of-the-art approaches, particularly under extreme noise conditions.
# Pipeline
