...
def robust_train(net, optimizer, trainloader, n_classes, candidate_count, config, train_loss_meter,train_accuracy_meter):
    ...
    for it, sample in enumerate(pbar):
        ...
        L_PLL = F.cross_entropy(logits, y_.cuda())
...
def build_loader(params):
    ...
    if dataset_name.startswith('cif'):
        ...
        trainloader = DataLoader(dataset['train'], batch_size=params.batch_size, shuffle=True, num_workers=16, pin_memory=True)
        test_loader = DataLoader(dataset['test'], batch_size=16, shuffle=False, num_workers=16, pin_memory=False)
